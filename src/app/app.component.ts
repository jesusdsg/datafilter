...
export class AppComponent {
  ...
  //Loading Aux JSON from local if Firebase is not available
  getLocalServices() {
    this.items = [];
    ...
    if (event.target.value) {
      this.items = this.items.filter(item => {
        if (!isNaN(event.target.value)) {
          return item.docnum.toString().indexOf(event.target.value) > -1;
        }
        else{
          return item.cardname.toLowerCase().includes(event.target.value.toLowerCase()) || item.firmname.toLowerCase().includes(event.target.value.toLowerCase()) || item.itemname.toLowerCase().includes(event.target.value.toLowerCase()) || item.slpname.toLowerCase().includes(event.target.value.toLowerCase())
        }
      });
    }
    else {
  ...
  setMarca() {
    ...
    this.sortJSON(this.items, 'firmname', 'asc');
  }
  setArticulo() {
    if (this.articulo) {
      this.articulo = false;
    ...
    this.sortJSON(this.items, 'itemcode', 'asc');
  }
  setCategoria() {
    if (this.categoria) {
    ...
    this.sortJSON(this.items, 'categoriacode', 'asc');
  }
  setCliente() {
    if (this.cliente) {
      this.cliente = false;
    ...
    this.sortJSON(this.items, 'cardcode', 'asc');
  }
